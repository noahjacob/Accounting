...
frappe.ui.form.on('Payment Entry', {
	setup(frm){
		...
		if(frm.doc.reference){
			...
			child.reference_type = "Purchase Invoice"
			child.reference_name = frm.doc.reference
			
			frappe.db.get_value('Purchase Invoice',frm.doc.reference,'total_amount').then(amount=>{
...
frappe.ui.form.on('Payment Entry Reference',{
	...
	amount(frm,cdt,cdn){
		console.log("After make payments")
		var total_amount = 0
		$.each(frm.doc.payment_references,function(i,row){
			total_amount = total_amount+row.amount
		...
		frm.refresh()
		console.log(frm.doc.amount_paid)
	},
